...
XGB_train_v7.py
-----------------------------------
Train an XGBoost regressor on **v7-processed calorimeter features**
to predict the logarithmic scaled energy ratio target:

    target = log( (100 × (E_true / Σ(E_layer_fractional))) + 1 )
...
Objective:
----------
v7 version applies a logarithmic transformation to the v6 target to reduce
skewness, stabilize training, and improve interpretability across a wide
dynamic range of event energies.

Outputs:
---------
- figures_v7/metrics_vs_epochs.png
- figures_v7/relative_error_vs_epochs.png
- figures_v7/feature_importance.png
- model_v7/xgb_model_v7.json

Author: Adapted from XGB_train_v6.py
"""
...
MODEL_DIR = "model"             # [CHANGED in v7]
FIGURES_DIR = "figures_v7"         # [CHANGED in v7]
VERBOSE_EVAL = 10
...
def load_processed_tensors():
    """
    Load processed features and labels from processed_data_0001_v4.pt (v7 input)

    [CHANGED in v7]
    Target is defined as:
        y = log( (100 × (E_true / Σ(E_layer_fractional))) + 1 )
    ...
    # [ADDED in v7] Use only first 28 columns for fractional layer energies
    sum_frac_layers = np.sum(X[:, :28], axis=1) + 1e-8  # avoid division by zero

    # [CHANGED in v7] Apply log-transform to the v6 target
    ratio = 100.0 * (y_true / sum_frac_layers)
    y = np.log(ratio + 1.0).astype(np.float32)
...
def train():
    ...
    print("Starting XGBoost training (v7 log-ratio target)...")
    bst = xgb.train(
        params=xgb_params,
        dtrain=dtrain,
    ...
    # [CHANGED in v7] Model save path
    model_path = os.path.join(MODEL_DIR, "xgb_model_7.json")
    ...
    print("\n=== Test Set Metrics (v7 log-ratio target) ===")
    print(f"MSE   : {mse:.6e}")
    ...
    plt.ylabel("Error")
    plt.title("Training and Validation Error vs Epoch (v7 log-ratio target)")
    plt.legend()
    plt.grid(True)
    ...
    plt.title("Validation Relative Errors vs Epoch (v7 log-ratio target)")
    plt.legend()
    plt.grid(True)
    ...
    plt.title("Feature Importance (v7 log-ratio target)")
    plt.tight_layout()
...
if __name__ == "__main__":
    results = train()
    print("\nTraining complete (v7 log-ratio target).")
