...
E_sum^(i) = Σ_{j=1}^{N_i} E_ij  
Where N_i is the number of hits in event i, and E_ij is the energy of hit j.
...
### 3. Energy-weighted Center-of-Gravity Standard Deviation in Radial Direction (r_std)
...
**Mathematical Formulation:**  
1. Compute COG:  
x_cog = Σ_j E_j x_j / E_sum,  y_cog = Σ_j E_j y_j / E_sum  
2. Radial distance of hit j:  
r_j = sqrt((x_j - x_cog)^2 + (y_j - y_cog)^2)  
3. Weighted standard deviation:  
r_std = sqrt(Σ_j E_j r_j^2 / E_sum - (Σ_j E_j r_j / E_sum)^2)
...
### 4. Energy-weighted Center-of-Gravity Standard Deviation in Z (z_std)
...
**Mathematical Formulation:**  
z_std = sqrt(Σ_j E_j (z_j - z_cog)^2 / E_sum - (Σ_j E_j (z_j - z_cog) / E_sum)^2)
...
r90 = r_k such that C_k ≥ 0.9 E_sum
...
**Mathematical Formulation:**  
E_layer_frac^(i) = Σ_{j in layer i} E_j / E_sum
...
### 7. Energy-Weighted Center-of-Gravity (x_cog, y_cog, z_cog)
...
**Mathematical Formulation:**  
x_cog = Σ_j E_j x_j / E_sum  
y_cog = Σ_j E_j y_j / E_sum  
z_cog = Σ_j E_j z_j / E_sum
...
| r_std | Transverse spread | √(Σ E_j r_j² / E_sum - (Σ E_j r_j / E_sum)²) | weighted_std(r) |
| z_std | Longitudinal spread | √(Σ E_j z_shift² / E_sum - (Σ E_j z_shift / E_sum)²) | weighted_std(z_shift) |
| r90 | 90% energy radius | r_k where Σ_{j≤k} E_j ≥ 0.9 E_sum | for-loop with cp.cumsum(E_sorted) |
| E_layer_frac[i] | Energy fraction per layer | Σ_{layer i} E_j / E_sum | cp.bincount(linear_idx, weights=E) / E_sum[:, None] |
